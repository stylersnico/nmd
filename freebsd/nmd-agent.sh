...
add_to_cron()
{
	...
	sleep 1
	if [ $FREQ -le 2 ]; then
		echo "0-59/$FREQ * * * * root sh /usr/local/nmd/nmd-agent.sh >> /var/log/nmd-agent.log 2>&1" >> $CRON
		echo "0 0 */$FREQLOG * 0 echo "" > /var/log/nmd-agent.log 2>&1" >> $CRON
	else
		let "START_MINUTE = $RANDOM % ($FREQ - 1)"
		let "START_MINUTE = $START_MINUTE + 1"
		let "END_MINUTE = 60 - $FREQ + $START_MINUTE"
		echo "$START_MINUTE-$END_MINUTE/$FREQ * * * * root sh /usr/local/nmd/nmd-agent.sh >> /var/log/nmd-agent.log 2>&1" >> $CRON
		echo "0 0 */$FREQLOG * 0 echo "" > /var/log/nmd-agent.log 2>&1" >> $CRON
	fi
	service cron restart
	echo
...
echo 'Connections | IP'
echo 
netstat -nTu4 | awk '{print $5}' | cut -d. -f1 -f2 -f3 -f4 | sort | uniq -c | sort -nr | $WHITE_LIST > $BAD_IP_LIST
cat $BAD_IP_LIST
echo
echo
echo '========================================================================='
